...
func run(t *testing.T, cfg goselfupdate.Config, options ...goclikit.Options) (string, error) {
	t.Helper()

	var out bytes.Buffer
	cmd := goclikit.UpdateCommand(cfg, options...)
	cmd.SetOut(&out)
	cmd.SetErr(&out)
...
func TestSkipChangelogOption(t *testing.T) {
	...
	out, err := run(t, config(source, "v1.0.0"), goclikit.Options{SkipChangelog: true})
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if strings.Contains(out, "Changes:") {
...
func TestDefaultNameAndNoAliases(t *testing.T) {
	cmd := goclikit.UpdateCommand(config(stubSource{tag: "v1.0.0"}, "v1.0.0"))
...
func TestOptionsOverrideName(t *testing.T) {
	cmd := goclikit.UpdateCommand(
		config(stubSource{tag: "v1.0.0"}, "v1.0.0"),
		goclikit.Options{Use: "self-update"},
	)
